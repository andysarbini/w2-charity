            ...
            <div class="col-lg-10">
                ...
                <form action="{{ url('/donation/'. $campaign->id .'/payment-confirmation/'. $donation->order_number) }}" method="POST" class="mt-3 mt-lg-4" enctype="multipart/form-data">
                    ...
                    <div class="form-group">
                        ...
                        @enderror" name="name" value="{{ old('name') ?? ($payment->name ?? $campaign->user->name) }}">
                            @error('name')
                    ...
                    <div class="form-group">
                        <label for="name">Jumlah Nominal Transfer <span class="text-danger">*</span></label>
                        <input type="text" class="form-control @error('nominal') is-invalid
                        @enderror" name="nominal" value="{{ old('nominal') ?? ($payment->nominal ?? $donation->nominal) }}">
                            @error('nominal')
                                <div class="invalid-feedback">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select name="bank_id" id="bank_id" class="form-control @error('bank_id') is-invalid
                        @enderror">
                    ...
                    <div class="form-group">
                        ...
                        <div class="custom-file">
                            <input type="file" class="custom-file-input @error('path_image') is-invalid
                            @enderror" id="path_image" name="path_image" onchange="preview('.preview-path_image', this.files[0])">
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="note" id="note" rows="4" class="form-control @error('note') is-invalid
                        @enderror">{{ old('note') ?? $payment->note }}</textarea>
                        @error('note')
                            <div class="invalid-feedback">
                                {{ $message }}
                            </div>
