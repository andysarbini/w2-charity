        ...
        <div class="row justify-content-center">
            <div class="col-lg-4">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('account') is-invalid @enderror" name="account" id="account" value="{{ old('account') ?? $setting->account }}">
                    @error('account')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <div class="col-lg-4">
                <div class="form-group">
                    ...
                    <input type="text" class="form-control @error('name') is-invalid @enderror" name="name" id="name" value="{{ old('name') ?? $setting->name }}">
                    @error('name')
                        <span class="invalid-feedback">{{ $message }}</span>
                    @enderror
                </div>
            ...
            <tr>
                ...
                <td>
                    <form action="{{ route('setting.bank.destroy', ['setting' => $setting->id, 'id' => $item->id]) }}" method="post">
                        ...
                        <button class="btn btn-link text-danger" onclick="return confirm('Yakin akan dihapus?')"><i class="fas fa-trash-alt"></i></button>
                    </form>
                </td>
            </tr>
        @endforeach
